# Migration Guide: OctoBot-Trading
...
### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `octobot_trading/` -> `src/octobot_trading/`
- **Import unchanged**: `import octobot_trading` still works

If upstream adds files to the old location, move them to `src/octobot_trading/`.
...
known-first-party = ["octobot_trading"]
```
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/octobot_trading/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import octobot_trading" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                        # Install all deps
uv run python -c "import octobot_trading"       # Verify import
